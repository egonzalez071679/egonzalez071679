# Hi, I'm Esteban Gonzalez👋

## Cybersecurity Professional in Training
### Project Management → IR/DFIR Analyst

---

## 🎯 About Me

Transitioning into cybersecurity from a 10+ year 
Project Management background. Specializing in 
Incident Response and Digital Forensics with a 
focus on in-house security teams.

Currently studying full time — 6 hours daily.

---

## 🏆 Certifications

| Status | Certification |
|--------|--------------|
| 🔄 In Progress | CCST Cybersecurity |
| ⏳ Planned | CCST Networking |
| ⏳ Planned | SC-900 Microsoft Security |
| ⏳ Planned | CCNA |
| ⏳ Planned | CyberOps Associate |
| ⏳ Planned | CompTIA Security+ |
| ⏳ Planned | BTL1 |
| ⏳ Planned | eCDFP |
| ⏳ Planned | eCIR |
| ⏳ Planned | CompTIA CySA+ |

---

## 🛠️ Skills & Tools

**Forensics:** Autopsy · FTK Imager · Volatility · SIFT  
**Network:** Wireshark · tcpdump · Nmap · Security Onion  
**SIEM:** Splunk · ELK Stack · Microsoft Sentinel  
**Frameworks:** MITRE ATT&CK · NIST CSF · Kill Chain  
**OS:** Linux · Windows  
**Languages:** Python (learning) · Bash (learning)  

---

## 📂 Repositories

| Repository | Description |
|------------|-------------|
| [tryhackme-writeups](https://github.com/egonzalez071679/tryhackme-writeups) | THM room solutions and learnings |
| [cybersecurity-labs](https://github.com/egonzalez071679/cybersecurity-labs) | Home lab exercises and documentation |
| [ir-reports](https://github.com/egonzalez071679/ir-reports) | Practice IR reports and investigations |
| [homelab-configs](https://github.com/egonzalez071679/homelab-configs) | Lab environment setup guides |

---

## 📈 Currently Working On

- CCST Cybersecurity — Month 1
- TryHackMe Pre-Security Path
- Linux Fundamentals daily practice

---

## 💼 Open To

GRC Analyst · Cybersecurity PM · SOC Analyst · IR/DFIR Analyst

---

## 📫 Connect With Me

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-blue)](https://linkedin.com/in/yourusername)
[![TryHackMe](https://img.shields.io/badge/TryHackMe-Profile-red)](https://tryhackme.com/p/yourusername)
[![Email](https://img.shields.io/badge/Email-Contact-green)](mailto:your@email.com)
